...
elseif ("${PLATFORM}" STREQUAL "SDL")
	...
	if(TARGET SDL3::SDL3)
		...
		# Prefer the static target when SDL3 is built as a subdirectory, so SDL
		# is embedded inside raylib and end users do not need a separate SDL3.dll.
		if(TARGET SDL3::SDL3-static)
			set(LIBS_PRIVATE SDL3::SDL3-static)
			message(STATUS "  -> using SDL3::SDL3-static (SDL embedded into raylib)")
		else()
	...
	elseif(TARGET SDL2::SDL2)
		...
		if(TARGET SDL2::SDL2-static)
			set(LIBS_PRIVATE SDL2::SDL2-static)
			message(STATUS "  -> using SDL2::SDL2-static (SDL embedded into raylib)")
		else()
			set(LIBS_PRIVATE SDL2::SDL2)
			message(STATUS "  -> using SDL2::SDL2 (shared)")
		endif()
		...
		if(SDL3_FOUND)
			...
			set(PLATFORM_CPP "PLATFORM_DESKTOP_SDL")
			# Prefer the static SDL3 target so SDL is embedded into raylib
			# (no runtime dependency on SDL3.dll / libSDL3.so / libSDL3.dylib).
			# Falls back to the shared target when the static lib is unavailable
			# (e.g. some Homebrew/macOS bottles ship only the shared library).
			if(TARGET SDL3::SDL3-static)
				set(LIBS_PUBLIC SDL3::SDL3-static)
				message(STATUS "  -> using SDL3::SDL3-static (SDL embedded into raylib)")
			else()
			...
			set(PLATFORM_CPP "PLATFORM_DESKTOP_SDL")
			if(TARGET SDL2::SDL2-static)
				set(LIBS_PUBLIC SDL2::SDL2-static)
				message(STATUS "  -> using SDL2::SDL2-static (SDL embedded into raylib)")
			else()
